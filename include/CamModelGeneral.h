...
public:
    ...
    void ConvertFisheye2Sphere_scara(cv::KeyPoint &fisheye, cv::Point3f &sphere);
    float GetSigma(const cv::KeyPoint &key);
...
inline void CamModelGeneral::WorldToImg(const cv::Vec3d& X,			// 3D scene point
    cv::Vec2f& m)			// 2D image point
{
    cv::Vec3d X_ = X/radius;
    double norm = cv::sqrt(X_(0)*X_(0) + X_(1)*X_(1));

    if (norm == 0.0)
        norm = 1e-14;

    const double theta = atan(-X_(2) / norm);

    const double rho = horner((double*)invP.data, invP_deg, theta);

    const double uu = X_(0) / norm * rho;
    const double vv = X_(1) / norm * rho;

    m(0) = round(uu*c + vv*d + u0);
    m(1) = round(uu*e + vv + v0);
}
    ...
    double& u, double& v) const							 // 2D image point
{
    ...
    const double theta = atan(-z_ / norm);
    const double rho = horner((double *)invP.data, invP_deg, theta);

    const double uu = x_ / norm * rho;
    const double vv = y_ / norm * rho;

    u = round(uu * c + vv * d + u0);
    v = round(uu * e + vv + v0);
}

...
void CamModelGeneral::ConvertFisheye2Sphere_scara(cv::KeyPoint &fisheye, cv::Point3f &sphere)
{
    //double invAff = c - d*e;
    const double u_t = fisheye.pt.x - u0;
    const double v_t = fisheye.pt.y - v0;

    // inverse affine matrix image to sensor plane conversion
    cv::Point3d world3d;
    world3d.x = (u_t - d * v_t) / invAffine;
    world3d.y = (-e * u_t + c * v_t) / invAffine;
    const double X2 = world3d.x * world3d.x;
    const double Y2 = world3d.y * world3d.y;
    world3d.z = -horner((double *)p.data, p_deg, sqrt(X2 + Y2));

    // normalize vectors spherically
    double norm = sqrt(X2 + Y2 + world3d.z * world3d.z);

    //world(3d) 2 sphere(3d)
    sphere.x = world3d.x / norm * radius;
    sphere.y = world3d.y / norm * radius;
    sphere.z = world3d.z / norm * radius;
}

float CamModelGeneral::GetSigma(const cv::KeyPoint &key)
{
    float x2 = (key.pt.x - u0) * (key.pt.x - u0) + (key.pt.y - v0) * (key.pt.y - v0);
    float x1 = sqrt(x2);
    float x3 = x2 * x1;
    float x4 = x2 * x2;
    float x5 = x4 * x1;
    ...
    return a5 * x5 + a4 * x4 + a3 * x3 + a2 * x2 + a1 * x1 + a0;
}

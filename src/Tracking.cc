...
* it under t he terms of the GNU General Public License as published by
...
void Tracking::Track()
{
    ...
    else /// initialization end -> tracking start
    {
        // cvWaitKey(0);
        bool bOK;
        ...
        if (!mbOnlyTracking) 
        {
            if (mState == OK)   ///tracking
            {
                ...
                {
                    bOK = TrackReferenceKeyFrame(); //done
                    // cout <<"TrackReferenceKeyFrame: " << bOK << endl;
                }
                else
                {
                    bOK = TrackWithMotionModel(); //done
            ...
            {
                // cout << "todo-> have not implemented the Relocalization() module" << endl;
                bOK = Relocalization(); //todo-> have not implemented the Relocalization() module 
        ...
        if (!mbOnlyTracking)
        {
            ...
            if (bOK)
            {
                bOK = TrackLocalMap(); //done
                // cout << "TrackLocalMap: " << bOK << endl;
                // cout << "num of map point (too less -> err):: " << mpMap->GetAllMapPoints().size() << endl;
            }
        } /////////////////////////////////
...
void Tracking::SearchLocalPoints()
{
    ...
    // cout << "befor [isInFrustum] orgin pt num:: " << mvpLocalMapPoints.size() << endl;
    // cout << "after [isInFrustum] nToMatch.size():: " << nToMatch << endl;
    ...
    if (nToMatch > 0)
    {
        ...
        matcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints, th); //1
    }
}
    ...
    while (nCandidates > 0 && !bMatch)
    {
        for (int i = 0; i < nKFs; i++)
        {
            ...
            if (!Tcw.empty())
            {
                ...
                if (nGood < 50)
                {
                    int nadditional = matcher2.SearchByProjection(mCurrentFrame, vpCandidateKFs[i], sFound, 10, 100); //4
                    ...
                    if (nadditional + nGood >= 50)
                    {
                        ...
                        if (nGood > 30 && nGood < 50)
                        {
                            ...
                            nadditional = matcher2.SearchByProjection(mCurrentFrame, vpCandidateKFs[i], sFound, 3, 64); //4
